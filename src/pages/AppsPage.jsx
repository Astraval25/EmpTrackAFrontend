...
const AppCard = ({ app }) => {
    ...
    <div className="neo-card overflow-hidden p-0">
      <div className={`bg-gradient-to-br ${app.color} p-8 text-white`}>
        <div className="flex items-start justify-between gap-4">
          <div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-white/15 overflow-hidden">
            {app.name && app.name.toLowerCase() === "coreflow" ? (
              <img src={coreflowImg} alt="CoreFlow" className="h-full w-full object-cover" />
            ) : (
              <Icon size={30} />
            )}
          </div>
  ...
  if (app.name && app.name.toLowerCase() === "coreflow") {
    return (
      <a
