...
import kioskSetupExe from "../assets/KioskLockSetup.exe";
...
export const DashboardPage = ({
  ...
  const handleDownloadKiosk = () => {
    const link = document.createElement("a");
    link.href = kioskSetupExe;
    link.download = "KioskLockSetup.exe";
      ...
      <section className="rounded-[32px] bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 p-8 text-white shadow-[0_30px_80px_rgba(15,23,42,0.22)]">
        <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex items-center gap-3 self-start">
            <button
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-5 py-3 text-sm font-black text-slate-900 transition-all hover:bg-slate-100"
              onClick={() => {
            ...
            <button
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-500 px-5 py-3 text-sm font-black text-white transition-all hover:bg-blue-600"
              onClick={handleDownloadKiosk}
            >
              <Download size={18} />
              <span>Download Kiosk</span>
            </button>
          </div>
        </div>
      ...
      <section id="add-user-modal" className="neo-card p-8 scroll-mt-8">
        ...
        <form onSubmit={handleCreateSubmit} className="grid grid-cols-1 gap-6 md:grid-cols-3 md:items-end">
          <div className="space-y-2">
            <label className="ml-1 text-[10px] font-bold uppercase tracking-[0.24em] text-slate-400">Employee Name</label>
            <input
              type="text"
